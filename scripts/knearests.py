...
def objective(trial):
    ...
    X_train_ = pd.read_csv(os.path.join(DATA_DIRECTORY,'processed', 'Merged', 'X_genus_shared_filteredLow_relative_genus_train.csv'), index_col=0)
    y_train = pd.read_csv(os.path.join(DATA_DIRECTORY,'processed', 'Merged', 'y_genus_shared_filteredLow_relative_genus_train.csv'), index_col=0).to_numpy().flatten()
...
#     taxonomic_level = trial.suggest_categorical('taxonomic level', ['Pylum','Class','Order','Family','Genus', 'Species'])
    ...
    regressor_obj = KNeighborsRegressor(**param)
...
if __name__ == "__main__":
	...
	#learn
	joblib.dump(study, os.path.join(DATA_DIRECTORY,'processed', 'Merged', 'X_genus_shared_filteredLow_relative_genus_train.csv', 'knearests_batchCorrected.pkl'))
